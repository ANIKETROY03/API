...
async function call() {
    ...
    let request = `https://api.nasa.gov/planetary/apod?date=` + getDate() + '&api_key=YOUR_API_KEY';    //Enter your API Key
    fetch(request).then(response => {
        return response.json();
    }).then(async function (myJSON) {
        //request = `https://api.nasa.gov/planetary/apod?date=` + getDate() + '&api_key=YOUR_API_KEY';
...
async function random() {
    spinner.removeAttribute('hidden');      //Spinner
    let request = `https://api.nasa.gov/planetary/apod?date=` + randomDate() + '&api_key=YOUR_API_KEY';     //Enter your API Key
    fetch(request).then(response => {
        return response.json();
    }).then(async function (myJSON) {
        //request = `https://api.nasa.gov/planetary/apod?date=` + getDate() + '&api_key=YOUR_API_KEY';
